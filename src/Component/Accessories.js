...
const Accessories = () => {
  const [accessoriesas, setAccessoriesas] = useState([
    data.Accessories.AUSTRALIA,
  ]);
  const [iphon, setIphon] = useState([data.Accessories.CANADA]);
  const [japan, setJapan] = useState([data.Accessories.JAPAN]);
  const [mexico, setMexico] = useState([data.Accessories.MEXICO]);
  const [india, setIndia] = useState([data.Accessories.INDIA]);

  return (
    <div>
      <h3 className="keys">Accessories</h3>
