...
const NA = () => {
  const [accessoriesas, setAccessoriesas] = useState([data.NA.AUSTRALIA]);
  const [iphon, setIphon] = useState([data.NA.CANADA]);
  const [japan, setJapan] = useState([data.NA.JAPAN]);
  const [mexico, setMexico] = useState([data.NA.MEXICO]);
  const [india, setIndia] = useState([data.NA.INDIA]);

  return (
    <div>
      <h3 className="keys">NA</h3>
