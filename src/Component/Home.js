...
const Home = () => {
  const [accessoriesas, setAccessoriesas] = useState([data.Home.AUSTRALIA]);
  const [iphon, setIphon] = useState([data.Home.CANADA]);
  const [japan, setJapan] = useState([data.Home.JAPAN]);
  const [mexico, setMexico] = useState([data.Home.MEXICO]);
  const [india, setIndia] = useState([data.Home.INDIA]);

  return (
    <div>
      <h3 className="keys">Home</h3>
